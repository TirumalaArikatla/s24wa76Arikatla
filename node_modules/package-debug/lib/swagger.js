...
module.exports = function () {
  ...
  function getBaseDir() {
    ...
    return base + `/${getKaName()}/${getGroup()}`;
  }
  ...
  function getNameSpace() {
    ...
    return nameSpace + `_${getKaName()}_${getGroup()}`;
  }
...
// map存储
  ...
  async function getDocs(group) {
    ...
    if (docs.info) {
      ...
      loading.succeed(`成功成功，DTO：${Object.keys(docs.definitions).length}个；接口：${Object.keys(docs.paths).length}个。`);
    }
  }
...
// response 返回值 类型处理
  ...
  // 处理 schema 的输出
  function getSchemaOutput(schema) {
    let arrayData = '[]';
    if (schema.type !== 'array') arrayData = '';
    return `${getSchemaTypeValue(schema)}${arrayData}`;
  }
...
// 替换数据类型 integer 和 枚举这种数据的处理
  function replaceVal(type, enums = []) {
    ...
    if (enums.length) {
      let res = '';
      enums.forEach((em, index) => {
        res += `'${em}'${index !== enums.length - 1 ? ' | ' : ''}`;
      });
      return res;
    }
    return type;
...
// 创建键值对 {key: value}
  function createKeyValue(key, schema, isRequired = false, tabCount = 2) {
    let txt = '';
    let description = `// ${replaceDescription(schema.description) ||
    '缺少description'}`;
    txt += tabTxt(description, tabCount);
    txt +=
      tabTxt(
        `${key}${isRequired ? '' : '?'}: ${getSchemaOutput(schema)};`,
        tabCount
      );
    return txt;
  }
...
// 获取 schema 的 类型的 值， 关联了DTO就取 DTO的值， 没有关联 就取基础数据 类型
  function getSchemaTypeValue(schema) {
    ...
    if (schema.additionalProperties)
      return getSchemaTypeValue(schema.additionalProperties);
    if (schema.originalRef) return importDto(schema.originalRef);
    if (schema.type) return replaceVal(schema.type , schema.enum ? schema.enum : []);
...
// 类型是否 关联了 DTO ？
  ...
  function addAnyProp(txt, tabCount = 2) {
    return txt += tabTxt(`[key: string]: any;`, tabCount);
  }
  ...
  function createAPi(paths) {
    try {
      ...
      for (let path in paths) {
        ...
        txt += tabTxt(
          `export const ${apiName} = (data?: ${requestParamsType}) => {`,
          0
        );
        txt += tabTxt(
